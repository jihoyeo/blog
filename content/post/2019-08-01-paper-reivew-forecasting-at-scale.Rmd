...
Prophet은 Facebook에서 개발한 Time-series prediction model이다. Deep learning이 핫한 이 시점에서 딥러닝을 사용하지 않은 모형이라는 점이 신기하다. 전통적인 ARIMA 계열의 시계열 모형과, 최근 널리 쓰이고 있는 LSTM, RNN과 같은 모형과 비교해 보았을 때 어떤 성능을 낼 수 있는지 잘 모르겠지만, 그래서 paper review 및 실습 코드를 활용해서 구현을 연습해 보고자 한다.

실습은 추후에 다른 포스팅에서 진행하도록 하고 (논문 작성을 위해 고속도로 데이터를 가지고 해보고자 한다). 여기서는 Facebook이 발표한 논문인 ['Forecasting at Scale'](https://www.tandfonline.com/doi/full/10.1080/00031305.2017.1380080) 논문에 대한 리뷰를 작성하고자 한다. 논문 본 글 외에도 [모든이들을 위한 FACEBOOK PROPHET PAPER 쉬운 요약 정리]("https://gorakgarak.tistory.com/1255")를 많이 참조하였다.
...
trend, seasnonality, holidays의 세개의 핵심 component로 구성되어 있고 다음과 같이 표현된다.

$y(t) = g(t)+s(t)+h(t)+\epsilon_t$

$g(t)$는 주가성이 없는 트렌트의 변화를 $s(t)$는 weekly, yearly와 같은 주기적인 변화를, $h(t)$는 holiday와 같은 불규칙적인 스케줄을 나타낸다. $\epsilon_t$는 에러텀으로 정규분포를 따른다고 가정한다.

이 모델은 Generalized Additive Model (GAM)과 비슷하다고 한다. GAM은 다른 포스트에 좀 더 상세히 설명되어 있다. 내가 이해하기론 regression인데 선형이 아닌 regression으로 이해하였다. 
...
Facebook 사용자의 증가는 인구의 증가형태와 매우 유사한 패턴을 보임. 즉, non-linear하며 carrying capacity에서 Saturation함.
...
한가지 중요한 다른 파라미터는 $C(t)$, time-varying capacity이다. 이 변수를 설정하는데는 분석자의 직관이 중요하다고 하는데, 구체적인 언급은 없다. 일단 핵심은, trend를 구할 때 changepoint를 설정하여 changepoint마다 growth rate와 capacity를 다르게 설정한다는 이야기 같다. 
...
where k is the growth rate, $\delta$ has the rate adjustments, m is the offset parameter, and $\gamma_j$ is set to $-s_j\delta_j$ to make the function continuous.  
수학적 수식을 정확하게는 모르겠지만 뒤에 $\gamma$와 함께나오는 식은 수식은 연속으로 만들어주기 위해 취해주는 것 같다. 이 수식은 증가나 감소에 제약이 없는 linear growth model임. 위에 있는건 logistic growth model.
...
이 논문에서는 $\delta_j~Laplace(0,\tau)$를 따른다고 하였다.
라플라스 분포에 대한 설명은 https://en.wikipedia.org/wiki/Laplace_distribution 이곳을 참고하자
...
> Incorporatingthislistofholidaysintothe modelismade straightforward by assuming that the effects ofholidays are independent.

For each holiday $i$, let $D_i$ be the set of past and future dates for that holiday.  
We add an indicator function representing whther time $t$ is during holiday $i$, 
...
It is often important to include effects for a window of days around a particular holiday, such as the weekend ofThanksgiving. 
...
When the seasonality and holiday features for each observation are combined into a **matrix X**
and **the changepoint indicators $a(t)$ in a matrix A**, the entire model of $y(t) = g(t)+s(t)+h(t)+\epsilon_t$ can be expressed in a few lines of Stan code. We use Stan’s L-BFGS to find a maximum a posteriori estimate
...
$\deltal$ is Magnitude of change of growth rate at changepoints  
...
해당 부분을 결정하는데 있어서는 **Insight and Domain knowledge**가 매우 중요하다. 이처럼 본 논문은 statistical model의 power만큼이나 분석자의 직관이나 domain knowledge가 중요하다는 것을 강조한다. 고속도로에서의 통행 수요의 경우에도 복잡한 예측 모형보다는 경험을 기반으로 한 모형이 더 정확하며, 네비게이션의 통행 시간을 산정해주는 알고리즘 역시 여러번의 수동적인 수정을 통해 현재와 같이 고도화 되었다고 들었다.

> Theforecasting literature oftenmakes thedistinction between statistical forecasts, which are based on models fit to historical data, and judgmental forecasts (also called manage- rial forecasts), which human experts produce using whatever process they have learned tends to work well for a specific time series. Each of these approaches has their advantages.
